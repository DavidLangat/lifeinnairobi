...
interface RestaurantsClientPageProps {
  heading: string;
  description: string;
  restaurants: {
    title: string;
...
export default function RestaurantsClientPage({ heading, description, restaurants }: RestaurantsClientPageProps) {
  return (
      ...
      <div className="relative z-10">
        ...
        >
          <div className="container mx-auto px-6 lg:px-12 max-w-7xl">
            <BentoGrid items={restaurants} />
          </div>
        </section>

        <ServiceCategories
          title="More Dining Options"
          items={restaurants.slice(0, 4).map(item => ({
              ...item,
              href: '#' // Placeholder as in original, or potentially valid links if available
          }))}
        />
