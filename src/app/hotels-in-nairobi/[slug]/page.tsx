...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
    ...
    return {
        ...
        alternates: {
            canonical: `https://nairobi.life/hotels-in-nairobi/${slug}`,
        },
        openGraph: {
            title: `${titleStr} | Places to Visit in Nairobi`,
            description: `Experience ${item.name} in Nairobi.`,
            url: `https://nairobi.life/hotels-in-nairobi/${slug}`,
            type: 'website',
            images: [
                {
                    url: item.image,
...
export default async function Page({ params }: { params: Promise<{ slug: string }> }) {
    ...
    const relatedItems = data.items
                        ...
                        .map(i => ({
                            title: i.name,
                            image: i.image,
                            href: `/hotels-in-nairobi/${i.slug}`
                        }));

    return <CategoryClientPage 
              item={item} 
              relatedItems={relatedItems} 
              basePath="/hotels-in-nairobi" 
              categoryName="Hotels in Nairobi" 
           />;
}
